...
#include <fstream>
#include <QSound>
RobotFirst::RobotFirst(QWidget *parent)
    : QMainWindow(parent)
    ...
    connect(bt, &QPushButton::clicked, this, &RobotFirst::sendSlot);
    bt->show();
    ...
    connect(rule, &QPushButton::clicked, this, &RobotFirst::show_rule);
    ...
    connect(stopgame, &QPushButton::clicked, this, &RobotFirst::defeat);
    ...
    connect(regret, &QPushButton::clicked, this, &RobotFirst::try_regret);
    ...
    connect(draw, &QPushButton::clicked, this, &RobotFirst::try_draw);
    //鼠标监听
    setMouseTracking(true);
    //设置窗口大小
    this->setFixedSize(1400, 740); //设置窗口大小
    setWindowTitle("五子棋-电脑先手");
    initGame();
}

void RobotFirst::sendSlot() {
    QSound *cl = new QSound(":/222/sound/click.wav");
    cl ->play();//播放函数
    emit closed();
}
void RobotFirst::defeat() {
    QSound *cl = new QSound(":/222/sound/click.wav");
    cl ->play();//播放函数
...
void RobotFirst::try_draw() {
    QSound *cl = new QSound(":/222/sound/click.wav");
    cl ->play();//播放函数
    QMessageBox::information(this, "失败", "电脑拒绝与你求和 ");
}
void RobotFirst::try_regret() {
    QSound *cl = new QSound(":/222/sound/click.wav");
    cl ->play();//播放函数
    if(player==WHITEFLAG&&step.size()) {
...
//绘制事件
void RobotFirst::paintEvent(QPaintEvent *) {
    //绘制棋盘边界边框，棋盘内颜色
    QPainter painter(this);
    painter.drawPixmap(QRect(0, 0, 1400, 740), QPixmap(":/222/images/robotfirst_bg.png"));
...
//鼠标移动事件
void RobotFirst::mouseMoveEvent(QMouseEvent *event) {
    selectPos = 0;
...
//鼠标点击事件
void RobotFirst::mouseReleaseEvent(QMouseEvent *event) {
    ...
    qDebug()<<Robotway::judgeChessSituation(dir);
...
//落子合法性判断
bool RobotFirst::judge_out_of_board(int x, int y) {
    if (x >= 15 || x < 0 || y >= 15 || y < 0) return 1;
    return 0;
}//是否越界

int RobotFirst::judge_pre_forbiden(int x, int y, int type) {
    if (type == BLACKFLAG) {
        std::vector<std::vector<int>>dir;
...
//-----------------判断行，列，对角线是否有威胁
//判断主对角线
std::vector<int> RobotFirst::main_diagonal(int x, int y, int type) {
...
//判断副对角线
std::vector<int> RobotFirst::counter_diagonal(int x, int y, int type) {
    std::vector<int>chess_type(9, 3);
    chess_type[4] = type;
    for (int i = 1; i <= 4; i++) {
        if (judge_out_of_board(x + i, y - i)) {
...
//判断行
std::vector<int> RobotFirst::row(int x, int y, int type) {
    std::vector<int>chess_type(9, 3);
    chess_type[4] = type;
    for (int i = 1; i <= 4; i++) {
...
//判断列
std::vector<int> RobotFirst::column(int x, int y, int type) {
    std::vector<int>chess_type(9, 3);
    chess_type[4] = type;
    for (int i = 1; i <= 4; i++) {
...
//分别计算攻击分数和防守分数
int RobotFirst::atk_val(int i, int j, int type) {
    std::vector<std::vector<int>>dir;
...
int RobotFirst::def_val(int i, int j, int type) {
    type = type % 2 + 1;
    std::vector<std::vector<int>>dir;
    dir.push_back(counter_diagonal(i, j, type));
    dir.push_back(main_diagonal(i, j, type));
...
//下一步得分和当前步加权计算
RobotFirst::value RobotFirst::cal_score(RobotFirst::value nxt, RobotFirst::value now) {
    return now + nxt;
}

void RobotFirst::Grade()//计算所有位置的得分
{
    int i, j, m, n;
    int num1, num2;
    for (i = 4; i < 19; i++)
...
void RobotFirst::Base(int x, int y)//坐标为(x-4,y-4)的所有五连珠情况
{
    //左上->右下
    for(int i=0;i<=4;i++) {
        for(int j=4;j>=0;j--) {
...
int RobotFirst::Assessment(int num1, int num2)//评分标准
{
    if (num2 == 0)//判断电脑五元组得分
    {
        switch (num1)
...
void RobotFirst::Find_point(int *pp, int *qq)//找到最佳性最优的下子位置
{
...
//深度优先搜索
std::pair<RobotFirst::value, std::pair<int, int>> RobotFirst::bfs(int type, int step, value now_score,value alpha,value beta) {
    std::vector<std::pair<RobotFirst::value, std::pair<int, int>>>res;
    bool if_rush = 0;
    bool if_die = 0;
...
void RobotFirst::playByAI() {
    ifcut = 0;
    std::pair<RobotFirst::value, std::pair<int, int>>computer = bfs(player, 0, {0, 0},{-INF,-INF},{INF,INF});
...
bool RobotFirst::judge_winner(int x, int y, int type) {
...
void RobotFirst::add_search_set(int x, int y) {
